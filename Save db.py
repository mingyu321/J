...
# MySQL 데이터베이스 연결 설정
# YOUR_HOST, YOUR_USER, YOUR_PASSWORD, YOUR_DATABASE를 본인의 MySQL 정보로 변경해주세요!
DB_CONFIG = {
    'host': 'localhost',        # 예: 'localhost' 또는 컨테이너 IP
    'user': 'root',             # 예: 'root' 또는 다른 MySQL 사용자
    'password': '',# MySQL 비밀번호 (이전에 설정한 값)
    'database': 'YOUR_DATABASE' # 미리 생성된 데이터베이스 이름 (예: 'crawling_db')
}
...
def get_article_links(list_url):
    ...
    for article in articles:
        try:
            ...
            # 기자가 없는 경우 처리 로직 추가 (reporter_element 활용)
